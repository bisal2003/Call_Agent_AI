...
import src.tools
from src.models import get_retriever, create_rag_chain, get_llm  # Import the necessary functions
...
# Initialize the retriever and RAG chain
...
# Route to handle agent response with RAG
...
def main_agent():
    ...
    # Generate agent response using RAG
    response = rag_chain.invoke({
        "query": conversation_history,  # Use the conversation history as the query
    ...
    clean_message = response["result"].split("<END_OF_TURN>")[0].strip() if "<END_OF_TURN>" in response["result"] else response["result"]
    isendofcall = response["result"].endswith("<END_OF_CALL>")
